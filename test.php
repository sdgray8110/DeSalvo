<?php
class facebookFeed {
    protected $postsObject, $photoObject;
    ...
    private function getCachedPhotos() {
        return file_get_contents($_SERVER['DOCUMENT_ROOT'] . '/cache/facebookPhotos.cache.json');
    }
    ...
    private function mikesPosts() {
        $feed = $this->decodeFeed($this->getCachedFeed());
        $feed = $feed['data'];
        $mikesPosts = array();
        ...
        $mikesPosts = $this->getPictures($mikesPosts);

        return $mikesPosts;
    }

    private function getPictures($feed) {
        $photos = $this->setPhotos();

        for ($i=0;$i<count($feed);$i++) {
            $photoID = $feed[$i]['object_id'];
            $feed[$i]['picture'] = $this->getPhotoURL($photoID, $photos);
        }

        return $feed;
    }

    private function getPhotoURL($photoID, $photos) {
    ...
    private function setPhotos() {
        $feed = $this->decodeFeed($this->getCachedPhotos());
        return $feed['data'];
    }
    ...
    public function cachePhotos() {
        $json = $this->getPhotoFeed();
        $cache = "facebookPhotos.cache.json";
        $fh = fopen($cache, 'w') or die("can't open file");
        fwrite($fh, $json);
        fclose($fh);
    }
