...
;(function($) {
	...
	$.extend($.superbox, {
        ...
        appendCloseButton : function() {
            if (!$innerbox.find('.close').length) {
                $.superbox.closeBtn = $('<a class="close" title="Close">Close</a>');
                $innerbox.prepend($.superbox.closeBtn);

                $.superbox.closeBtn.click(function() {
                    $.superbox.close();
                });
            }
        },
